import React from 'react'
import { featureLists, goodLists } from '../../constans'
import { useMediaQuery } from 'react-responsive'
import { useGSAP } from '@gsap/react';
import gsap from 'gsap';

const Art = () => {

    const isMobile = useMediaQuery({ maxWidth: 767 });

    useGSAP(() => {
        const start = isMobile ? 'top 20%' : 'top top';

        const maskTimeline = gsap.timeline({
            scrollTrigger: {
                trigger: '#art',
                start: start,
                end: '+=1000',
                scrub: 1.5,
                pin: true,

            },
        });

        maskTimeline.to('.will-fade', {
            opacity: 0,
            stagger: 0.2,
            ease: "power1.inOut",
        })
            .to('.masked-img', {
                scale: 1.3,
                maskPosition: 'center',
                maskSize: '400%',
                duration: 8,
                ease: "power1.inOut"
            })
            .from('.cup-right', {
                xPercent: 300,
                ease: "power1.inOut",
                duration: 2,
            })
            .from('.cup-left', {
                xPercent: -300,
                ease: "power1.inOut",
                duration: 2,
            }, '<')
            .to('#masked-content', {
                opacity: 1,
                duration: 2,
                ease: "power1.inOut"
            })

    });

    return (
        <div id='art'>
            <div className='container mx-auto h-full pt-20 relative'>
                <img src="/images/drink1.png" alt="" className='hidden md:block absolute bottom-45 right-0 w-50 cup-right rotate-350' />
                <img src="/images/drink3.png" alt="" className='hidden md:block absolute top-45 left-0 w-45 cup-left rotate-10' />
                <h2 className='will-fade'>The ART</h2>
                <div className='content'>
                    <ul className='space-y-4 will-fade'>
                        {goodLists.map((feature, index) => (
                            <li key={index} className='flex items-center gap-2'>
                                <img src="/images/check.png" alt="check" />
                                <p>{feature}</p>
                            </li>
                        ))}
                    </ul>
                    <div className='cocktail-img'>
                        <img src="/images/under-img.jpg" alt="cocktail" className='abs-center masked-img size-full object-contain' />
                    </div>
                    <ul className='space-y-4 will-fade'>
                        {featureLists.map((feature, index) => (
                            <li key={index} className='flex items-center justify-start gap-2'>
                                <img src="/images/check.png" alt="check" />
                                <p className='md:w-fit w-60'>{feature}</p>
                            </li>
                        ))}
                    </ul>
                </div>
                <div className='masked-container'>
                    <h2 className='will-fade'>Perfection in Every Sip</h2>
                    <div id='masked-content'>
                        <h3>Born of Skill, Served with Heart</h3>
                        <p>Beyond just a drink. It’s a precision-made experience crafted for you.</p>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Art
